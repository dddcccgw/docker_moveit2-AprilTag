...
import time
import numpy as np
from . import d435_helpers as dh
...
def main(use_remote_computer, exposure):
    try: 
        ...
        if use_remote_computer:
            address = 'tcp://*:' + str(yn.d435_port)
        else:
            address = 'tcp://' + '127.0.0.1' + ':' + str(yn.d435_port)
        ...
        while True:
            ...
            if first_frame:
                ...
                d435_output = {'depth_camera_info' : depth_camera_info,
            ...
            d435_output['color_image'] = color_image
            d435_output['depth_image'] = depth_image
                
            socket.send_pyobj(d435_output)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog='Send D435 Images',
        description='Send D435 images to local and remote processes.'
    )
    parser.add_argument('-r', '--remote', action='store_true', help='Use this argument when allowing a remote computer to receive D435 images. Prior to using this option, configure the network with the file forcesight_networking.py on the robot and the remote computer.')
    parser.add_argument('-e', '--exposure', action='store', type=str, default='low', help=f'Set the D435 exposure to {dh.exposure_keywords} or an integer in the range {dh.exposure_range}') 
