...
from . import d435_helpers_without_pyrealsense as dh
...
def main(use_remote_computer: bool, x: int, y: int, push_radius_m: float,
    ...
    # Subscriber to D435 image stream
    d435_context = zmq.Context()
    d435_socket = d435_context.socket(zmq.SUB)
    d435_socket.setsockopt(zmq.SUBSCRIBE, b'')
    d435_socket.setsockopt(zmq.SNDHWM, 1)
    d435_socket.setsockopt(zmq.RCVHWM, 1)
    d435_socket.setsockopt(zmq.CONFLATE, 1)
    if use_remote_computer:
        d435_address = 'tcp://' + yn.robot_ip + ':' + str(yn.d435_port)
    else:
        d435_address = 'tcp://' + '127.0.0.1' + ':' + str(yn.d435_port)
    d435_socket.connect(d435_address)
    ...
    try:
        while True:
            ...
            d435_output = d435_socket.recv_pyobj()
            color_image = d435_output.get('color_image')
            depth_image = d435_output.get('depth_image')
            depth_camera_info = d435_output.get('depth_camera_info')
            depth_scale = d435_output.get('depth_scale')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--remote', action='store_true', help='Run on remote computer (bind publisher, subscribe to robot D435).')
    parser.add_argument('-x', type=int, required=True, help='Pixel x (columns) in the D435 color/depth image space.')
    parser.add_argument('-y', type=int, required=True, help='Pixel y (rows) in the D435 color/depth image space.')
